    ...
    <div className="overflow-x-auto">
      <div className="min-w-[920px]">
        <div className="grid grid-cols-[88px_repeat(7,minmax(92px,1fr))] gap-2">
          <div className="flex items-center gap-2 rounded-md border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs font-medium text-zinc-400">
          ...
          {days.map((day) => (
            <div
              key={day}
              className="rounded-md border border-zinc-800 bg-zinc-900 px-3 py-2 text-center text-xs font-medium text-zinc-300"
            >
              {day.slice(0, 3)}
            </div>
          ))}
          ...
          {times.map((time) => (
            <Fragment key={time}>
              <div
                key={`${time}-label`}
                className="rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 font-mono text-xs text-zinc-500"
              >
                {time}
              </div>
              {days.map((day) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      !slot && "border-zinc-900 bg-zinc-950 text-zinc-700",
                      slot?.status === "booked" &&
