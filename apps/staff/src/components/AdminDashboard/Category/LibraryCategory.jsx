...
const LibraryCategory = () => {
    ...
    fetch(`${API_BASE}/category`)
      ...
      .then((data) => {
        if (data?.data?.length > 0) {
          setCategories(data.data[0].library || []);
        }
      })
  ...
  const onSubmit = (data) => {
    setLoading(true);
    axios
      .put(`${API_BASE}/category/library/68af1a2bda8917c50135cebc`, data)
      .then(() => {
        toast.success("Library category added successfully");
        reset();
  ...
  const handleDelete = (id) => {
    ...
    axios
      .put(`${API_BASE}/category/librarydelete/${id}`, data)
      .then((res) => {
        if (res.status === 201 || res.status === 200) {
          toast.success("Library category deleted");
          setRefreshTrigger((prev) => !prev);
        }
      })
      .catch((err) => {
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
      ...
      <div className="md:col-span-5 bg-slate-50/50 border border-slate-100 rounded-2xl p-6 h-fit space-y-4">
        <div className="flex items-center gap-2 text-emerald-800">
          ...
          <h3 className="font-bold text-sm">Add Library Category</h3>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. হাদিস বিষয়ক বই"
      ...
      <div className="md:col-span-7 space-y-3">
        <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Active Library Categories ({categories.length})</h3>
