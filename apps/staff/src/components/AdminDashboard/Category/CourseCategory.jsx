...
const CourseCategory = () => {
    ...
    fetch(`${API_BASE}/category`)
      ...
      .then((data) => {
        if (data?.data?.length > 0) {
          setCategories(data.data[0].course || []);
        }
      })
      .catch((err) => console.error("Error loading course categories:", err));
  }, [refreshTrigger, API_BASE]);

  const onSubmit = (data) => {
    setLoading(true);
    axios
      .put(`${API_BASE}/category/course/68af1a2bda8917c50135cebc`, data)
      .then(() => {
  ...
  const handleDelete = (id) => {
    ...
    axios
      .put(`${API_BASE}/category/coursedelete/${id}`, data)
      .then((res) => {
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
      ...
      <div className="md:col-span-5 bg-slate-50/50 border border-slate-100 rounded-2xl p-6 h-fit space-y-4">
        <div className="flex items-center gap-2 text-emerald-800">
          ...
          <h3 className="font-bold text-sm">Add Course Category</h3>
        </div>
      ...
      <div className="md:col-span-7 space-y-3">
        <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Active Course Categories ({categories.length})</h3>
